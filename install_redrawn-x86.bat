...
:: Install Git
:installlgit
powershell -Command "Invoke-WebRequest https://github.com/git-for-windows/git/releases/download/v%GIT_VERSION%.windows.2/Git-%GIT_VERSION%-32-bit.exe -OutFile git_installer.exe"
call git_installer.exe
echo Git Has Been Installed. Checking dependencies again...
timeout 4
goto dependency_check
:installnode
powershell -Command "Invoke-WebRequest https://nodejs.org/dist/v%NODE_VERSION%/node-v%NODE_VERSION%-x86.msi -OutFile node_installer.msi"
...
goto dependency_check
:installchromium
powershell -Command "Invoke-WebRequest https://github.com/tangalbert919/ungoogled-chromium-binaries/releases/download/79.0.3945.130-2/ungoogled-chromium_79.0.3945.130-2.1_installer-x86.exe -OutFile chromium_installer.exe"
